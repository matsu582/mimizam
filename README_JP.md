...
3. **尺度不変指紋**: アンカー＋2ターゲットの三つ組から32bitビットパックハッシュを生成。時間比 `(t1-tA)/(t2-tA)` と周波数比 `log2(f1/fA)`, `log2(f2/fA)` を符号化するため、速度変化（時間伸縮）・ピッチ変化（周波数スケール）に本質的に不変。暗号学的ハッシュではない。（旧「絶対周波数・絶対時間差 `[f1][f2][Δt]`」方式を完全に置き換え。既存DBは再指紋化が必要）
4. **単一検索マッチング**: DB検索は1回のみ（time/freq スケールのブルートフォース探索なし）。その後ハフ投票による頑健直線回帰 `db ≈ slope·query + offset` で time_scale/offset を復元し、整列インライアから信頼度を算出
...
4. **フレーム単位ANN投票**: クエリの各フレームがバックエンドのベクトルANNで近傍を取得し、映像別に得票/類似度を集計
5. **幾何検証（再ランク）**: 上位候補について、クエリとDBのフレームをAKAZE記述子の幾何検証（BF Hamming最近傍＋Loweの比率検定＋ホモグラフィRANSAC/USAC_MAGSACのインライア数）で突き合わせる。検証済みペアをDB時刻の近さでクラスタして一致区間を導く（PiP矩形検出付き）
