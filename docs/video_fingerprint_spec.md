...
コードブックが適切でないと、VLAD集約の品質が低下する。例えば、アニメ映像で学習したコードブックを実写映像に適用すると、実写特有の微妙なテクスチャ差異を捉えられず、異なる映像の指紋が似通ってしまう。
...
対象ドメインの映像群（例: アニメ全14本）からキーフレームを選定し、各フレームのAKAZE記述子を全て収集する。典型的な学習データ量は数十万〜数百万個の61次元記述子となる。
...
推論時は `(VLADベクトル - pca_mean) × pca_components^T` で128次元に変換する。
...
```
映像群（14本）
  ↓ フレーム選定（各映像500〜1200フレーム）
約15,000フレーム
  ↓ AKAZE記述子抽出（各フレーム500〜2000個）
約1,500万個の61次元記述子
  ↓ K-Means (k=64, MiniBatchKMeans)
64個の61次元中心ベクトル = コードブック
  ↓ 全フレームのVLADベクトル生成
約15,000個の3,904次元ベクトル
  ↓ PCA学習 (128次元)
128×3,904の主成分行列 + 3,904次元の平均ベクトル
  ↓ 保存
モデルファイル（.pki）
```
