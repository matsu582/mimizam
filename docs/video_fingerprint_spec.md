...
シーン検出には PySceneDetect の `ContentDetector` を用いる。`ContentDetector` は各フレームをHSV色空間へ変換し、隣接する評価フレーム間で色相・彩度・明度の平均的な変化量を求めて1つのコンテンツスコアに統合する。別のショットへの切り替え（カット）では画面全体の色や明るさが大きく変わるためスコアが跳ね上がり、これが閾値（`scene_threshold` = 27.0）を超えた評価フレームを「シーン変化（新しいショットの先頭）」と判定する。ゆるやかなパンやズームのようにスコアが閾値を超えない変化はシーン変化とはみなさない。
...
VLADベクトル（3,904次元）は高次元すぎて保存・検索に非効率であるため、PCA（主成分分析）で512次元に圧縮する。なお PCA の出力次元はコードブック学習ツール側で 128/256/512 から選べるが、本映像指紋では 512 次元を採用している。
...
登録時に生成・保存するのは**フレーム単位指紋**である。選定された各フレームについて、AKAZE記述子 → VLAD集約 → PCA圧縮 → L2正規化 の順に処理し、1フレーム=1ベクトルの指紋を得る。フレーム指紋は512次元である。
...
幾何検証を通過したペアは「同一画」である。似たカット（繰り返しや類似構図のショット）が続く区間では、クエリの各フレームが同一区間内の別カットに一致し得るため、時間オフセット（DB時刻 − クエリ時刻）はフレーム毎にばらつく。一方、一致先のDB時刻はその区間（DB上の連続した一定の時間帯）に集中する。
...
複数フレームの画素値の時間分散（各画素が時間方向にどれだけ変化するか）と、連続フレーム間差分の平均を計算する。PiP小窓内と背景では動きのパターンが異なるため、分散マップや差分マップに矩形状の境界が現れる。

候補矩形は次の2手法を併用して生成する:

- **プロジェクション方式**: 分散マップの水平・垂直方向の平均プロファイルから急変点を検出し、矩形の境界候補を作る。
- **2D連結成分解析**: 分散マップを複数の閾値で二値化し、連結成分のバウンディングボックスを矩形候補にする。

候補はそのまま採用せず、重複除去後に**包含・高IoUの候補群を外接矩形へ統合**する。PiP内部の強い動きだけが断片的な高スコア矩形になる場合でも、断片候補を1つのPiP全体枠へ戻してから評価するため、検索対象が小窓の一部に偏りにくい。

統合後の候補に対し、次の3要素を組み合わせて `pip_score` を算出する:

- 矩形内外の時間分散の差
- 矩形境界の内側帯と外側帯におけるフレーム間差分の不連続性
- 矩形内外のフレーム間差分量の差

`pip_score` が閾値（`pip_score_threshold` = 0.5）以上の候補をPiP矩形として残し、検索では `pip_score` が最も高い1件（`pip_max_regions` = 1）だけを使用する。複数の断片候補をそのまま検索に流すと偽陽性が混ざりやすいため、最もPiPらしい全体枠に絞る。

**PiP矩形検出例**（緑枠: 検索に使用するPiP矩形）:

![PiP矩形検出例](pip_detection_example.png)

### 4.3 PiP矩形内指紋生成

検出された矩形領域を各サンプルフレームから切り出し、長辺1280pxにアップスケールしてからAKAZE記述子を抽出する。アップスケールにより、縮小された映像でもDB側のフレーム指紋（フルサイズで生成）と近いスケールの特徴量が得られる。

PiP矩形内でも通常フレームと同じ形式で指紋を作る:

```text
PiP矩形内フレーム
  ↓ AKAZE特徴点 + 記述子
  ↓ VLAD集約 → PCA(512次元) → L2正規化
PiP矩形フレーム指紋群
```

幾何検証に使う生記述子も通常経路と同じ `N×(2+D)` 形式（先頭2列がキーポイント座標、残りがAKAZE記述子）で保持する。これにより、PiP経路でも通常経路と同じRANSAC検証を適用できる。

### 4.4 PiP検索と統合

PiP経路は、矩形内フレーム指紋に対して通常経路と同じ照合手順を使う。つまり、PiP専用にANN類似度だけで順位を決めるのではなく、候補絞り込み後にフレーム単位の幾何検証を通し、被覆率・得票率を反映した実効映像スコアで統合する。

```text
PiP矩形フレーム指紋群
  ↓ ANN候補絞り込み（矩形内の各フレームで投票）
  ↓ フレーム単位の幾何検証（AKAZE BF/RANSAC）
  ↓ 一致区間抽出 + 実効映像スコア化
通常経路の結果と同じスコア基準で統合
```

この構成により、全画面フレームが背景に引きずられて一致しない場合でも、PiP矩形内の映像だけを元動画と照合できる。一方で、PiP経路の生ANN類似度だけがランキングを支配することを避け、通常経路と同じ幾何検証済みスコアで比較する。
...
**学習データ**: 全フレームのVLADベクトル（各3,904次元）を行列として並べ、分散が最大になる方向（主成分）を512個抽出する。

**PCAの動作**:

1. 全VLADベクトルの平均を計算 → `pca_mean`（3,904次元）
2. 平均を引いた共分散行列を計算
3. 固有値分解で分散の大きい方向512個を抽出 → `pca_components`（512×3,904行列）

推論時は `(VLADベクトル - pca_mean) × pca_components^T` で圧縮する。

**分散保持率**: 3,904次元から512次元に圧縮しても、元データの分散の大部分が保持される。これは、VLADベクトルの多くの次元が相関を持っており（例: 隣接クラスタの残差は似た傾向を示す）、少数の主成分で大部分の情報を表現できるためである。
...
| `pca_dimensions` | 512 | 映像指紋で使用するPCA圧縮後の次元数（学習ツールでは128/256/512を選択可能だが、本映像指紋では512次元を採用） |
...
| `pip_max_regions` | 1 | 検索に使用するPiP矩形数。`pip_score` 最上位1件のみを使用 |
