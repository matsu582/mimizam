...
### 3.2 フレーム単位マッチングと時間オフセットクラスタリング

候補映像に対して、クエリの各フレーム指紋とDB側フレーム指紋を照合し、各クエリフレームについて「DB側で最も類似するフレーム」とそのタイムスタンプを記録する。ここから「どの部分が一致しているか」を特定する。
...
**位置推定は最大整列クラスタを採用する**: 時間差を離散化（ビン化）し、最も件数の多いビン＝最大整列クラスタの中心を代表オフセットとする。全編に散らばる偶発一致（ノイズ）に左右されず、真の一致がノイズより少数でも正しい位置を報告できる。音声側の位置推定（`fingerprint_database._calculate_time_offset`）および表示用の一致区間も同じ最大整列クラスタ方式に統一している。
...
同一UUIDで登録された音声指紋・映像指紋を突き合わせ、統合スコアを算出する（`examples/movie_search.py`）。

```
両モダリティ一致（かつ位置が乖離しない）: 統合 = √(音声信頼度 × 実効映像スコア)   （幾何平均）
片方のみ / 位置乖離あり:                  統合 = max(音声信頼度, 実効映像スコア) × 0.8
```

統合スコア 0.3 未満は結果から除外する。

**位置乖離チェック**: 同一映像で音声・映像の両方が一致しても、音声が指すDB区間と映像が指すDB区間が大きく離れている（隙間 > 30秒）場合、別々の箇所での偶発一致であり「音声＋映像の二重一致」とは認めない。この場合は幾何平均で持ち上げず単独モダリティ評価に切り替え、表示に「※音声位置が映像と乖離（二重一致から除外）」を明記する。これにより、誤整列由来の薄い一致が二重一致で過大評価される偽陽性を抑制する。
