...
クエリ映像の指紋ベクトルとDB内全映像のベクトルのドット積を計算し、類似度の高い上位K件を候補とする。数百次元のドット積計算は非常に高速であり、数万件のDBでもミリ秒で完了する。
...
コードブックは「視覚パターンの辞書」であり、対象ドメイン（アニメ等）に特化する必要はない。COCOとUCF-101の多様な画像・動画で学習することで、エッジ・角・テクスチャ等の基本的な視覚パターンを網羅的にカバーでき、あらゆるジャンルの映像に対して安定した指紋を生成できる。
...
| `video_fingerprints` | 映像全体指紋（N次元ベクトル、デフォルト512） | 映像数 × 2KB |
| `frame_fingerprints` | フレーム単位指紋（N次元 × フレーム数） | 映像数 × 数MB |
