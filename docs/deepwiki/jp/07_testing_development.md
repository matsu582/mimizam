...
開発環境では、コア機能に加えてテストインフラとオプションのデータベースバックエンドを含む包括的なセットアップが推奨されます。

#### インストールオプション
- **完全開発環境**: 全データベースバックエンドとテストツールを含む
- **基本開発環境**: SQLiteバックエンドのみの軽量セットアップ
- **特定バックエンド**: 必要なデータベースのみを選択的にインストール
...
音声指紋エンジンの各段階を包括的に検証するテストスイートです。

##### テスト対象
- **スペクトログラム生成**: 周波数・時間軸の正確性と非負値検証
- **ピーク検出精度**: 既知周波数（A4音440Hz等）の検出確認
- **ハッシュ生成**: 指紋ハッシュの一意性と再現性
- **適応的パラメータ**: 音声特性に応じた動的調整の検証

#### データベース層テスト

各データベースバックエンドの機能を統一的に検証するパラメータ化テストです。

##### テスト範囲
- **指紋保存・取得**: 全バックエンド（SQLite、MySQL、PostgreSQL）での一貫性
- **検索性能**: 大量データでの検索速度とスケーラビリティ
- **データ整合性**: トランザクション処理と例外処理の検証
- **接続管理**: 接続プールと同時アクセスの安定性
