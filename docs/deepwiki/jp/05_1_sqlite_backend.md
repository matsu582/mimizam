...
if os.path.exists("music.db"):
    ...
    print("新しいデータベースファイルが作成されます")

# ファイルサイズの確認
if os.path.exists("music.db"):
    size = os.path.getsize("music.db")
    print(f"データベースサイズ: {size / 1024 / 1024:.2f} MB")
```
...
def enable_wal_mode(db_path):
    ...
    conn.close()
    print("WALモードを有効化しました")

# 使用例
enable_wal_mode("music.db")
```
