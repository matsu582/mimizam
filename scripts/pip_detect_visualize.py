...
さらに、検出された矩形が真のPiP全体ではなく一部しか覆えないケースの診断用に、
候補矩形の後処理戦略 A/B/C を切り替え、各戦略が選ぶ代表矩形（USED）を並べて
目視比較できるようにしている。

戦略:
    baseline  包含統合なし（断片候補がそのまま残る旧挙動の確認用）
...
使い方:
    uv run ./scripts/pip_detect_visualize.py <動画パス>
    uv run ./scripts/pip_detect_visualize.py --strategy all <動画パス>
    uv run ./scripts/pip_detect_visualize.py --strategy A,B <動画パス> --cluster-margin 0.03
...
# 検出パイプラインの内部関数を再利用する。候補生成とスコアリングを本体と
# 同じ条件に揃えつつ、戦略差分だけをこの診断ツール側で切り替える。
